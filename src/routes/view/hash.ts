...
export const hash = async (req: Request) => {
    ...
    if (!url.searchParams.get('id')) return new Response(JSON.stringify({ success: false, message: "no ?id" }), { status: 404 });
    ...
    const responseHeaders = 'headers' in dataRaw ? {headers: { ...dataRaw.headers as {}, ...cacheHeaders }} : { headers: cacheHeaders };
