...
export async function imageUpload(formData: FormData, req: Request, context: ImageUploadContext): Promise<any> {
    const file = formData.get('file') as File
    const blurhash = formData.get('blurhash') as string
    const id = nanoid()

    const { uploadDataList, generatedHash } = await createUploadData(file, id, blurhash, context.contentType);
