...
export async function videoUploadHandler(formData: FormData, req: Request, context: VideoUploadContext): Promise<any> {
    const id = nanoid()
    const file = formData.get('file') as File
    const blurhash = formData.get('blurhash') as string
    const fileData = await file.arrayBuffer();

    const response = await videoUpload(Buffer.from(fileData), id, context.contentType, blurhash)

    return Response.json(response)
}

export const videoUpload = async (data: Buffer, id: string, type: string, blurhash?: string) => {
    let promises = [uploadFileData(data, id, type, blurhash), extractFrameAndUpload(data)];
...
const uploadFileData = async (fileData: Buffer, id: string, contentType: string, blurhash: string) => {
