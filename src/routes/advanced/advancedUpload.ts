...
export const advancedUpload = async (request: Request) => {
    if (request.method === 'GET') {
        return Response.json({
            ...
            usage: '?type=[multi-track|multi-track-fast] in the URL for different types of processing.',
            note: 'Use multi-track for longest processing (does not clip) and multi-track-fast for shortest processing (clips content).'
        }, {status: 200});
    }

    if (request.method !== 'POST') return Response.json({success: false, message: 'Please post'}, {status: 405})
    ...
    switch (url.searchParams?.get('type')) {
        ...
        default:
            return Response.json({success: false, message: "Could not find searchParams, or type"})
    }
}