...
export const imagesAndAudio = async (req: Request) => {
  ...
  if (!imageFiles || !imageFiles[0])
    return Response.json(
      {
        success: false,
        message: "no image provided in Form body",
      },
      { status: 404 },
    );
  ...
  for (let i = 0; i < imageFilePaths.length; i++) {
    filters.push(
      `[${i}:v]scale=w=1080:h=1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2:color=black[v${i}]`,
    );
  }

  filters.push(
    `${new Array(imageFilePaths.length)
      .fill("")
      .map((v, i) => `[v${i}]`)
      .join("")}concat=${imageFilePaths.length}:v=1:v=1[v]`,
  );
  ...
  ffmpegProcess.on("close", async (code) => {
    ...
    if (code == 0) {
      ...
      await videoUpload(outputData, id, "video/mp4");
    }
