...
export async function extractFrameAndUpload(file: Blob): Promise<Buffer> {
    return new Promise(async (resolve, reject) => {
        ...
        const onCompletion = () => {
            // Delete the files
            fs.unlink(filepath, err => { if(err) console.error(`Error deleting file ${filepath}:`, err); });
            fs.unlink(outpath, err => { if(err) console.error(`Error deleting file ${outpath}:`, err); });
        };
